...
export class ObjectController {
	...
	public CheckCollide(Collider: BoxCollider, Object: _OBJBase) {
		...
		if (Center.sub(this.Client.Position).magnitude <= Collider.size.magnitude * 3) {
			const [Min, Max] = [Center.sub(Collider.size.mul(0.5)), Center.add(Collider.size.mul(0.5))];
	...
	public CollideWithClient() {
		...
		if (LastPosition !== this.Client.Position) {
			...
			const [Cast, _1, _2, Collider] = Physics.SphereCast(
				LastPosition,
				this.Client.Config.Radius * this.Client.Config.Scale,
				Look.normalized,
				Look.magnitude,
				Constants().Masks().ObjectLayer,
			);
