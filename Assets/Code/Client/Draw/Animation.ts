...
export class Animation {
	...
	constructor(
		...
		RigParent: Transform,
		public AnimList: typeof Animations,
		private Controller: Animator,
		public DrawInfo: DrawInformation,
	) {
		this.Last = "Idle";
		this.Current = "Fall";
	...
	private UpdateState(Animation: InferredAnimation, Playing: boolean, TransitionTime?: number) {
		if (Playing) {
			...
			for (const [Key, Value] of pairs(Animation)) {
				...
				this.Controller.CrossFadeInFixedTime(Value.Name, TransitionTime ?? 0.2, Key);
	...
	private CalculateWeightAndSpeed(Animation: InferredAnimation, Initial: boolean = false) {
		for (const [Key, Value] of pairs(Animation)) {
			...
			if (Value.Position !== undefined) {
				this.WeightLayers[Key as 0].Target = this.GetCurrentTrack(Animation)[0] === Value.Name ? 1 : 0.01;

				if (Initial) this.WeightLayers[Key as 0].Current = this.WeightLayers[Key as 0].Target;
			}
